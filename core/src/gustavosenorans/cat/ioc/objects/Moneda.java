...
public class Moneda extends Scrollable {
    ...
    public Moneda(float x, float y, float width, float height, float velocity) {
        super(x, y, width, height, velocity);

        collisionCircle = new Circle();

        r = new Random();
        assetMoneda = r.nextInt(8);
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        batch.draw(AssetManager.moneda[assetMoneda], position.x, position.y, this.getOriginX(), this.getOriginY(), width, height, this.getScaleX(), this.getScaleY(), this.getRotation());
    }

    public boolean collidesMonedas(Doll nau) {
